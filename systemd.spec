...
install -dm0755 %{buildroot}/usr/lib/firewalld/services/
install -Dm0644 -t %{buildroot}/usr/lib/firewalld/services/ %{SOURCE7} %{SOURCE8}

# Restore systemd-user pam config from before "removal of Fedora-specific bits"
install -Dm0644 %{SOURCE12} %{buildroot}/etc/pam.d/systemd-user

# Install additional docs
# https://bugzilla.redhat.com/show_bug.cgi?id=1234951
install -Dm0644 %{SOURCE9} %{buildroot}%{_pkgdocdir}/%{SOURCE9}

# https://bugzilla.redhat.com/show_bug.cgi?id=1378974
install -Dm0644 %{SOURCE10} %{buildroot}%{system_unit_dir}/systemd-udev-trigger.service.d/%{SOURCE10}

install -Dm0755 %{SOURCE11} %{buildroot}%{_prefix}/lib/kernel/install.d/%{SOURCE11}

install -D %{SOURCE3} %{buildroot}/usr/lib/systemd/purge-nobody-user
